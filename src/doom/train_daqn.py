...
def learn(criterion, agent, autoencoder, optimizer, s1, target_q):	
...
def get_q_values(agent, autoencoder, state):
	...
	_, code = autoencoder(state)
	#image_save(state, img_output)
	output = agent(code)

	return output

def get_best_action(agent, autoencoder, state):
	...
	q = get_q_values(agent, autoencoder, state)
	m, index = torch.max(q, 1)
	action = index.cpu().data.numpy()[0]
	return action

def learn_from_memory(memory, batch_size, agent, discount_factor, 
					  criterion, optimizer, actual_epoch_loss_vector, autoencoder):
	""" Learns from a transition, that comes from the replay memory.
	s2 is ignored if isTerminal equals true """

	if memory.size > batch_size:
		s1, a, s2, isterminal, r = memory.get_sample(batch_size)    
		q = get_q_values(agent, autoencoder, s2).cpu().data.numpy() 
		q2 = np.max(q, axis=1)
		target_q = get_q_values(agent, autoencoder, s1).cpu().data.numpy()
		target_q[np.arange(target_q.shape[0]), a] = r + discount_factor * (1 - isterminal) * q2

		loss = learn(criterion, agent, autoencoder, optimizer, s1, target_q)
...
def perform_learning_step(num_epochs, game, actions, resolution, frame_repeat, memory, batch_size, 
					  agent, discount_factor, criterion, optimizer, actual_epoch_loss_vector, autoencoder, epoch):
	...
	for epoch in range(config.epochs):
		...
		for learning_step in trange(config.learning_steps_per_epoch, leave=False):
			...
			perform_learning_step(config.epochs, config.game, config.actions, config.resolution, config.frame_repeat, 
								  ...
								  config.criterion, config.optimizer, config.actual_epoch_loss_vector, config.autoencoder, epoch)
		...
		for test_episode in trange(config.test_episodes_per_epoch, leave=False):
			...
			while not config.game.is_episode_finished():
				state = preprocess(config.resolution, config.game.get_state().screen_buffer)
				state = state.reshape([1, config.resolution[0], config.resolution[1], config.resolution[2]])
				best_action_index = get_best_action(config.agent, config.autoencoder, state)
